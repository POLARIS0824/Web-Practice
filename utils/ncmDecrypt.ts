/**
 * NCM (NetEase Cloud Music Encrypted Audio) In-Browser Decryptor
 * 纯前端内存中实时解密 NCM 文件为可播放的 MP3/FLAC Blob URL (100% 独立内置 AES 引擎，零外部依赖)
 */

// Round constant words for AES-128
...
function convertToInt32(bytes: Uint8Array): number[] {
  const result: number[] = [];
...
function createAes128KeySchedule(keyBytes: Uint8Array): number[][] {
  const Kd: number[][] = [];
  for (let i = 0; i <= 10; i++) Kd.push([0, 0, 0, 0]);
  const tk = convertToInt32(keyBytes);
  for (let i = 0; i < 4; i++) {
    Kd[10][i] = tk[i];
  }
  let rconpointer = 0;
  ...
  for (let r = 1; r < 10; r++) {
    for (let c = 0; c < 4; c++) {
      const tt = Kd[r][c];
      Kd[r][c] = (U1[(tt >> 24) & 0xff] ^ U2[(tt >> 16) & 0xff] ^ U3[(tt >> 8) & 0xff] ^ U4[tt & 0xff]);
    }
  }
  return Kd;
}

function decryptAesBlock(ciphertext: Uint8Array, offset: number, Kd: number[][], output: Uint8Array, outOffset: number): void {
  const a = [0, 0, 0, 0];
  const t = [
    (ciphertext[offset] << 24) | (ciphertext[offset + 1] << 16) | (ciphertext[offset + 2] << 8) | ciphertext[offset + 3],
    (ciphertext[offset + 4] << 24) | (ciphertext[offset + 5] << 16) | (ciphertext[offset + 6] << 8) | ciphertext[offset + 7],
    (ciphertext[offset + 8] << 24) | (ciphertext[offset + 9] << 16) | (ciphertext[offset + 10] << 8) | ciphertext[offset + 11],
    (ciphertext[offset + 12] << 24) | (ciphertext[offset + 13] << 16) | (ciphertext[offset + 14] << 8) | ciphertext[offset + 15],
  ];
  for (let i = 0; i < 4; i++) t[i] ^= Kd[0][i];
  for (let r = 1; r < 10; r++) {
    for (let i = 0; i < 4; i++) {
      a[i] = (T5[(t[i] >> 24) & 0xff] ^ T6[(t[(i + 3) % 4] >> 16) & 0xff] ^ T7[(t[(i + 2) % 4] >> 8) & 0xff] ^ T8[t[(i + 1) % 4] & 0xff] ^ Kd[r][i]);
    }
    t[0] = a[0]; t[1] = a[1]; t[2] = a[2]; t[3] = a[3];
  }
  for (let i = 0; i < 4; i++) {
    const tt = Kd[10][i];
    output[outOffset + 4 * i] = ((Si[(t[i] >> 24) & 0xff] ^ (tt >> 24)) & 0xff);
    output[outOffset + 4 * i + 1] = ((Si[(t[(i + 3) % 4] >> 16) & 0xff] ^ (tt >> 16)) & 0xff);
    output[outOffset + 4 * i + 2] = ((Si[(t[(i + 2) % 4] >> 8) & 0xff] ^ (tt >> 8)) & 0xff);
    output[outOffset + 4 * i + 3] = ((Si[t[(i + 1) % 4] & 0xff] ^ tt) & 0xff);
  }
}

function aes128EcbDecrypt(data: Uint8Array, key: Uint8Array): Uint8Array {
  const Kd = createAes128KeySchedule(key);
  const blockCount = Math.floor(data.length / 16);
  const decrypted = new Uint8Array(blockCount * 16);
  for (let b = 0; b < blockCount; b++) {
    decryptAesBlock(data, b * 16, Kd, decrypted, b * 16);
  }
  if (decrypted.length > 0) {
    ...
    if (pad > 0 && pad <= 16) {
      ...
      for (let i = decrypted.length - pad; i < decrypted.length; i++) {
        if (decrypted[i] !== pad) { isPad = false; break; }
      }
      if (isPad) return decrypted.slice(0, decrypted.length - pad);
    }
  }
  return decrypted;
}
...
/**
 * 实时解密 NCM ArrayBuffer (100% 独立无依赖)
 */
export const decryptNcm = async (buffer: ArrayBuffer): Promise<DecryptedNcmResult> => {
  ...
  const decryptedKey = aes128EcbDecrypt(rawKeyData, CORE_KEY);
  ...
  if (metaLen > 0) {
    try {
      ...
      const decryptedMeta = aes128EcbDecrypt(encryptedMeta, META_KEY);
